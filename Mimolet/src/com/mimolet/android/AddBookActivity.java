...
public class AddBookActivity extends FragmentActivity{
  ...
  public void stylePageTabClick(View view) {
    ...
    if (imageFolder.exists()) {
      ...
      if (files.length != 0) {
        ...
        Toast.makeText(getApplicationContext(), R.string.no_image_found, Toast.LENGTH_LONG).show();
      }
    } else {
      Toast.makeText(getApplicationContext(), R.string.no_image_found, Toast.LENGTH_LONG).show();
    }
  ...
  public void previewTabClick(View view) {
    if (isTabAccessible(PREVIEW_TAB)) {
      ...
      final ImageView leftImage = (ImageView) previewFragment.getView().findViewById(R.id.previewLeftImage);
      final ImageView rightImage = (ImageView) previewFragment.getView().findViewById(R.id.previewRightImage);

      final File imageFolder = new File(GlobalVariables.IMAGE_FOLDER);
      stylePageFragment.setImagePathes(imageFolder.list());

      System.out.println(GlobalVariables.IMAGE_FOLDER + imageFolder.list()[0]);
      ImageUtils.loadImage(leftImage, GlobalVariables.IMAGE_FOLDER + imageFolder.list()[0], 300, true);
      ImageUtils.loadImage(rightImage, GlobalVariables.IMAGE_FOLDER + imageFolder.list()[1], 300, true);
    }
  }
  ...
  private void loadPreviewImages() {
    final ImageView bookCover = (ImageView) stylePageFragment.getView().findViewById(R.id.previewImage);
    final ImageView leftImage = (ImageView) stylePageFragment.getView().findViewById(R.id.leftImage);
    final ImageView rightImage = (ImageView) stylePageFragment.getView().findViewById(R.id.rightImage);

    final File imageFolder = new File(GlobalVariables.IMAGE_FOLDER);
    stylePageFragment.setImagePathes(imageFolder.list());
    ImageUtils.loadImage(bookCover, GlobalVariables.IMAGE_FOLDER + stylePageFragment.getPreviewImagePath());
  ...
  private void loadLeftRightImages() {
    final ImageView leftImage = (ImageView) stylePageFragment.getView().findViewById(R.id.leftImage);
    final ImageView rightImage = (ImageView) stylePageFragment.getView().findViewById(R.id.rightImage);
    ImageUtils.loadImage(leftImage, GlobalVariables.IMAGE_FOLDER + stylePageFragment.getLeftImagePath(), 300, true);
    ImageUtils.loadImage(rightImage, GlobalVariables.IMAGE_FOLDER + stylePageFragment.getRightImagePath(), 300, true);
  }
